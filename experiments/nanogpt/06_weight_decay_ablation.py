...
def run(args):
    ...
    base_dir = args.output_dir or os.path.join(PROJECT_ROOT, "results", "06_weight_decay_ablation")
    ...
    print(f"[06] wd={args.weight_decay} | {args.optimizer} | seed={args.seed}", flush=True)
    print(f"[06] CMD: {' '.join(cmd)}", flush=True)
    print(f"[06] cwd: {os.path.join(PROJECT_ROOT, 'nanoGPT')}", flush=True)
    t0 = time.time()
    ...
    print(f"[06] Subprocess done: rc={result.returncode} elapsed={elapsed:.1f}s "
    ...
    if result.returncode != 0:
        print(f"[06] SUBPROCESS FAILED (rc={result.returncode}). Last 80 stderr lines:", flush=True)
        for line in result.stderr.splitlines()[-80:]:
            print(f"  [stderr] {line}", flush=True)
        print(f"[06] Last 40 stdout lines:", flush=True)
