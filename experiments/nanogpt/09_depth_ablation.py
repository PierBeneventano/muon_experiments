...
def run(args):
    ...
    print(f"[09] n_layer={args.n_layer} | {args.optimizer} | seed={args.seed}", flush=True)
    print(f"[09] CMD: {' '.join(cmd)}", flush=True)
    print(f"[09] cwd: {os.path.join(PROJECT_ROOT, 'nanoGPT')}", flush=True)
    t0 = time.time()
    ...
    print(f"[09] Subprocess done: rc={result.returncode} elapsed={elapsed:.1f}s "
    ...
    if result.returncode != 0:
        print(f"[09] SUBPROCESS FAILED (rc={result.returncode}). Last 80 stderr lines:", flush=True)
        for line in result.stderr.splitlines()[-80:]:
            print(f"  [stderr] {line}", flush=True)
        print(f"[09] Last 40 stdout lines:", flush=True)
