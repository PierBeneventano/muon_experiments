...
def run(args):
    ...
    base_dir = args.output_dir or os.path.join(PROJECT_ROOT, "results", "07_momentum_ablation")
    tag = f"mom{args.momentum}_s{args.seed}"
    ...
    print(f"[07] momentum={args.momentum} | seed={args.seed}", flush=True)
    print(f"[07] CMD: {' '.join(cmd)}", flush=True)
    print(f"[07] cwd: {os.path.join(PROJECT_ROOT, 'nanoGPT')}", flush=True)
    t0 = time.time()
    ...
    print(f"[07] Subprocess done: rc={result.returncode} elapsed={elapsed:.1f}s "
    ...
    if result.returncode != 0:
        print(f"[07] SUBPROCESS FAILED (rc={result.returncode}). Last 80 stderr lines:", flush=True)
        for line in result.stderr.splitlines()[-80:]:
            print(f"  [stderr] {line}", flush=True)
        print(f"[07] Last 40 stdout lines:", flush=True)
