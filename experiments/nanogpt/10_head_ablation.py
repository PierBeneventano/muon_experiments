...
def run(args):
    ...
    print(f"[10] n_head={args.n_head} | {args.optimizer} | seed={args.seed}", flush=True)
    print(f"[10] CMD: {' '.join(cmd)}", flush=True)
    print(f"[10] cwd: {os.path.join(PROJECT_ROOT, 'nanoGPT')}", flush=True)
    t0 = time.time()
    ...
    print(f"[10] Subprocess done: rc={result.returncode} elapsed={elapsed:.1f}s "
    ...
    if result.returncode != 0:
        print(f"[10] SUBPROCESS FAILED (rc={result.returncode}). Last 80 stderr lines:", flush=True)
        for line in result.stderr.splitlines()[-80:]:
            print(f"  [stderr] {line}", flush=True)
        print(f"[10] Last 40 stdout lines:", flush=True)
