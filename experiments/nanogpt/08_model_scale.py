#!/usr/bin/env python3
"""
08_model_scale.py -- Does Muon's advantage scale with model width?
Sweep: --n_embd {64, 128, 256, 512}

Usage:
  python experiments/nanogpt/08_model_scale.py \
      --n_embd 256 --optimizer muon --seed 42

SLURM: one (n_embd, optimizer, seed) triple per job.
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Model width scaling")
    p.add_argument("--n_embd", type=int, required=True, choices=WIDTHS)
...
def run(args):
    ...
    base_dir = args.output_dir or os.path.join(PROJECT_ROOT, "results", "08_model_scale")
    tag = f"{args.optimizer}_embd{args.n_embd}_s{args.seed}"
    out_dir = os.path.join(base_dir, tag)
    os.makedirs(out_dir, exist_ok=True)

    # n_head must divide n_embd
    n_head = min(4, args.n_embd // 16)  # 64->4, 128->4, 256->4, 512->4
    n_head = max(n_head, 1)
    n_layer = 4
    block_size = 256
    ...
    print(f"[08] Done in {elapsed:.1f}s -> {summary_path}")
    return meta
